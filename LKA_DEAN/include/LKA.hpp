...
const Point UV_0(u_0, v_0);//图像平面中心点；
const Point UV_1(u_0, 2*v_0-60);//图像平面底部中点坐标；
const Point UV_2(u_0, 0);//图像顶部中心点；
...
const Mat camera_matrix1 = (Mat_<double>(3, 3) << 160.92, 0.0, 158,
											0.0, 215.98, 116,
											0.0, 0.0, 1.0);
const Mat dist_coeffs1 = (Mat_<double>(5, 1) << -0.15858, -0.00058 , -0.00139 , 0.00156 ,0.00000);
/*---------------------------------------------------------*/
const int stateNum = 4;	//卡尔曼滤波的状态和测量向量的维度
const int measureNum = 2;
const Point pt_0(160, 140);	//区域选择点；
const Point Vanish(160, 55);	//消失点；
const int Leftborder = 45, Rightborder = 285;//消失点左右边界；
const int predict_shift =15;
const int predict_y1 = 100, predict_y2 = 300;
const int resetframe = 10;
